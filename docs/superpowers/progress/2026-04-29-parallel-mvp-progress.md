...
- Android 原型视觉还原：按用户反馈确认 Android 截图来自 Expo Go 低保真 fallback，不能作为视觉验收口径；新增 `android:dev-build` / `android:device` 脚本与 Android package `com.newme.mobile`，明确 Android 视觉验收必须使用 Dev Build。
- 能量页原生视觉补强：新增 `EnergySlider.native.tsx`，Dev Build 下动态加载 `@shopify/react-native-skia` 绘制金色胶囊能量条、柔光、扫光、尾雾、粒子和喷口光效；Expo Go 保留纯 RN fallback。`PrototypeShell` 原生端改为深林色柔和渐变叠层，禁止硬圆片和网格线；`PrototypeBottomNav` 补 Android native shadow/elevation 和贴底 padding。
- TDD 记录：先在 `apps/mobile/tests/prototype-parity.spec.js` 新增 Android Dev Build 脚本、原生 EnergySlider Skia、原生背景层、底栏 native shadow 4 个防回归测试，红测确认分别失败于脚本缺失、文件缺失、背景层缺失和底栏原生样式缺失；实现后 focused 4 个用例通过。
- 文档同步：更新 `产品需求设计文档.md`、`docs/prototype-parity/2026-05-04-full-page-diff-audit.md`，新增 `docs/testing/2026-05-08-android-dev-build-visual-smoke.md`，统一“Expo Go 只做功能 smoke，Android Dev Build 才能做视觉验收”的口径。
- 验证记录：`pnpm --filter @newme/mobile typecheck` 通过；`node prototype/prototype-regression.test.cjs` 通过；`node prototype/prototype-interaction-smoke.cjs` 通过；`npx playwright test apps/mobile/tests/prototype-parity.spec.js --reporter=line --workers=1 --timeout=60000 --output=.tmp/pw-mobile-output` 21 个用例通过；`npx playwright test apps/mobile/tests/prototype-visual-regression.spec.js --reporter=line --workers=1 --timeout=60000 --output=.tmp/pw-mobile-output` 1 个用例通过；`pnpm --filter @newme/mobile exec expo export --platform web --output-dir dist-web` 通过；`pnpm --filter @newme/mobile exec expo config --type public` 确认 `android.package = com.newme.mobile`。
- Android Dev Build 截图验证：已确认 Android SDK/adb 可用但未加入 PATH，adb 实际路径为 `C:\Users\Administrator\AppData\Local\Android\Sdk\platform-tools\adb.exe`；用 Android Studio JBR 和短路径临时 worktree `C:\n` 成功构建安装 `com.newme.mobile` Dev Build，重启 Metro dev-client 并配置 `adb reverse tcp:8081 tcp:8081`、`adb reverse tcp:37200 tcp:37200`；前台 Activity 为 `com.newme.mobile/.MainActivity`，不是 Expo Go。登录后进入能量页，UI 树确认 `energy-orb`、`plasma-energy-slider`、`prototype-bottom-nav` 存在且 Skia 画布为 `TextureView`；截图已留存到 `test-results/android-dev-build-visual-smoke/energy-dev-build-2026-05-08.png`。
- Android 背景/能量球/能量条二次修正：针对用户复核指出的背景硬几何、能量球黑盘/硬圆、能量条廉价 HUD 感，先补 `prototype-parity.spec.js` 红测，要求 Android 原生背景不得包含 `nativeGridLines`/`prototype-native-grid-line`/`nativeRadialGlow`，并要求 `EnergyOrb.native.tsx`、`EnergySlider.native.tsx` 的 Skia Canvas 使用 `opaque={false}`，Android Skia 能量条不得渲染硬 HUD 角标/横线；实现后 focused 用例转绿。已重启 Metro dev-client、配置 adb reverse、用开发验证码登录并通过 `newme://energy` 打开能量页，截图留存到 `test-results/android-dev-build-visual-smoke/energy-deeplink-3.png`。
- Android 视觉复核三次修正：针对用户继续指出的“脏背景、清单顶部标签变形、能量条硬扫光”，新增/扩展红测，要求原生背景只保留 `prototype-native-ambient-backdrop` 柔和渐变，禁止大面积背景几何层；要求 Skia 能量条不再出现实心白色 Rect 扫光；要求清单 focus chip 保持低饱和 matte 标签。实现后重启 Metro dev-client、用开发验证码登录，分别 deep link 到能量页和清单页，最终有效登录态截图留存为 `test-results/android-dev-build-visual-smoke/2026-05-08-clean-bg/energy-soft-sweep-auth.png` 与 `test-results/android-dev-build-visual-smoke/2026-05-08-clean-bg/todo-soft-chips-auth.png`。
- 本轮复核验证记录：`pnpm --filter @newme/mobile exec playwright test tests/prototype-parity.spec.js --grep "native Android energy visuals|native energy bar|native energy orb|native prototype shell|todo focus chips"` 7 个 focused 用例通过；`pnpm --filter @newme/mobile typecheck` 通过；`pnpm --filter @newme/mobile exec expo export --platform web --output-dir dist-web` 通过。完整 `prototype-parity.spec.js` 在 `http://127.0.0.1:37300` 静态服务下尝试重跑但 180 秒超时，未作为通过依据；残留 Playwright 进程已清理。
- Android 背景根因修正尝试：用户再次复核指出“是不是只会写前端 CSS”，当时判断 `PrototypeShell` Android 端仍用 `expo-linear-gradient` 模拟原型背景并尝试改为 Skia 背景；该尝试后续被下一条“Android 背景色带收口”废弃，因为设备复核证明 Skia 透明径向渐变才是大圆弧/黑色锯齿的直接来源。历史截图留存为 `test-results/android-dev-build-visual-smoke/2026-05-08-skia-bg/energy-skia-bg-auth.png` 与 `test-results/android-dev-build-visual-smoke/2026-05-08-skia-bg/me-skia-bg-auth.png`，不再作为当前有效验收证据。
- Android 背景色带收口：针对用户截图里的清单页/我的页横向脏色带，补跑 Web 原型/客户端截图对照，确认 Web 基准为连续深林绿背景，不存在大圆弧、黑色锯齿或分层脏块；截图输出到 `test-results/web-compare-2026-05-08/`。复核确认 Android 背景层继续使用 Skia 透明 `RadialGradient` 会把透明径向层混合成大块脏形状，即使 `Canvas opaque={false}` 也不足以作为可靠验收路径；本轮直接移除 `PrototypeNativeBackground.native.tsx` 背景 Skia，改为 `expo-linear-gradient` 全屏纵向渐变、顶部线性柔光和底部弱绿色线性收尾，禁止背景层出现 `@shopify/react-native-skia`、`Canvas`、`RadialGradient`、`Rect`。已重启 Metro dev-client、强停并重新打开 `com.newme.mobile`，再用开发验证码登录后 deep link 到业务页截图，留存为 `test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-restart-auth/energy-linear-bg-auth.png`、`test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-restart-auth/todo-linear-bg-auth.png`、`test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-restart-auth/me-linear-bg-auth.png`；验证 `pnpm --filter @newme/mobile typecheck` 通过，`pnpm --filter @newme/mobile exec playwright test tests/prototype-parity.spec.js --grep "native prototype background" --reporter=line --workers=1 --timeout=60000 --output=../../.tmp/pw-mobile-output` 1 个 focused 用例通过，`pnpm --filter @newme/mobile exec playwright test tests/prototype-visual-regression.spec.js --reporter=line --workers=1 --timeout=120000 --output=../../test-results/web-compare-2026-05-08-final` 1 个 Web 对照用例通过。
- Android 背景断层继续收口：针对用户最新指出“还有明显断层”，确认上一版 `#020504` 底色叠加 `rgba(0, 0, 0, 0.24)` 全屏黑暗角会把中下段压成突兀纯黑。已改为与 Web 原型一致的 `#091411 -> #060b0a -> #030605` 纵向深林绿底，取消强黑暗角，仅保留底部极弱绿色线性收尾；`prototype-parity.spec.js` 增加 `#020504` 和 `rgba(0, 0, 0, 0.24)` 防回归断言。已清缓存重启 Metro dev-client、强停并打开 `com.newme.mobile`，登录已完成 onboarding 的本地测试号后复核能量/清单/我的页，截图留存为 `test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-continuity/energy-continuity.png`、`test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-continuity/todo-continuity-final.png`、`test-results/android-dev-build-visual-smoke/2026-05-08-linear-bg-continuity/me-continuity-now.png`；验证 `pnpm --filter @newme/mobile typecheck` 通过，`pnpm --filter @newme/mobile exec playwright test tests/prototype-parity.spec.js --grep "native prototype background" --reporter=line --workers=1 --timeout=60000 --output=../../.tmp/pw-mobile-output` 1 个 focused 用例通过。
- 临时构建目录收口：失败尝试产生的 `C:\nmbuild` worktree 和 `temp/android-visual-smoke` 分支已清理，`N:` subst 映射已移除；成功构建用的 `C:\n` / `temp/android-visual-smoke-short` 暂时保留，作为 Windows 长路径 CMake/Ninja 问题的 Android 原生构建工作区。
