...
- 2026-05-05 能量条升级：旧原型的圆头氮气条不再作为验收基准，已按用户目标图升级为金色胶囊能量条。新的真源要求暗金卡片、金色胶囊主体、深色未填充轨道、细金色外框、青色 HUD 端点/细线、克制粒子和默认 `62%` 目标态；尾部光雾必须渐隐消失，不能出现实体方框边界；移动端同步增加结构断言，防止退回普通 slider、独立圆形 thumb 或再次出现 `NaN%`。
